...
def encode_data(model, data_loader, log_step=10, logging=logger.info, backbone=False):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        if not backbone:
            img_emb_esa, cap_emb_esa, _, _ = model.forward_emb(images, captions, lengths, image_lengths=image_lengths)

        else:
            img_emb, cap_emb = model.forward_emb(images, captions, lengths)

        # Concatenate image and text embeddings for transformer encoder
        combined_features = torch.cat((img_emb_esa, cap_emb_esa), dim=1).to(img_emb_esa.device)
        combined_features = combined_features.unsqueeze(0)

        # Encode using transformer
        img_emb_trans, cap_emb_trans = model.transformer_encoder(combined_features)

        # Combine ESA and Transformer outputs
        img_emb = 0.9*img_emb_esa + 0.1*img_emb_trans
        cap_emb = 0.9*cap_emb_esa + 0.1*cap_emb_trans
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    tokenizer = BertTokenizer.from_pretrained('/home/s1/ESA-main6/bert-base-uncased')
