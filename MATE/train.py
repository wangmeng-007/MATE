...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        model.train_fusion(images, captions, lengths, img_lengths=img_lengths)

        # Update the momentum models
        model.update_momentum()  # 动量模型更新
...
def validate(opt, val_loader, model):
    ...
    # caption retrieval
    npts = img_embs.shape[0] #npts 代表的是图像和文本对的数量
